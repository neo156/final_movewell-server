      ...
      { userId: req.userId, date: today },
      {
        $push: {
          habitsCompleted: {
            habitId,
            title,
            actual, // Include actual value if provided
            goal,   // Include goal if provided
          },
        },
      },
      { new: true, upsert: true }
...
app.get('/api/progress/stats', verifyToken, async (req, res) => {
  try {
    ...
    // Calculate steps from 10K Steps habit if recorded
    let totalSteps = todayProgress?.steps || 0;
    if (todayProgress?.habitsCompleted) {
      const stepsHabit = todayProgress.habitsCompleted.find(h => 
        h.title?.includes('Steps') || h.title?.includes('10K')
      );
      if (stepsHabit?.actual) {
        totalSteps = stepsHabit.actual;
      }
    }
    ...
    const stats = {
      today: {
        ...
        habits: todayProgress?.habitsCompleted || [], // Include full habit data
      },
      streak: {
